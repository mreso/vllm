...
def get_config_heuristic(M, K, N):
    ...
    """
    # Determine M bucket (small: <2048, medium: 2048-4095,
    # large: 4096-8191, very_large: 8192+)
    m_bucket = ("very_large" if M >= 8192 else
                "large" if M >= 4096 else "medium" if M >= 2048 else "small")

    # Create parameter configuration using nested dictionaries
    configs = {
        # Format: {N: {m_bucket: (BLOCK_M, BLOCK_K, num_warps,
        # num_stages, waves_per_eu, kpack)}}
        16: {
    ...
    BLOCK_M, BLOCK_K, num_warps, num_stages, waves_per_eu, kpack = configs[N][
        m_bucket]
...
def _fused_routing_topk1(
    ...
    # (will be in float32 due to default acc_type)
    ...
    for k in range(0, K, BLOCK_K):
        ...
        W_ptrs = W_ptr + (offs_k_iter[:, None] * stride_wk +
        ...
        # Compute partial matmul for the current block using
    ...
    topk_weights = tl.sigmoid(topk_weights)
    ...
    if FUSED_SHARED_EXPERTS:
        ...
        topk_ids_buffer = tl.where((offs_topk[None, :] < _TOPK - 1),
                                   topk_ids[:, None], N)
        topk_weights_buffer = tl.where((offs_topk[None, :] < _TOPK - 1),
                                       topk_weights[:, None], 1.0)
    else:
        topk_ids_buffer = topk_ids[:, None]
        topk_weights_buffer = topk_weights[:, None]

    topk_ids_ptrs = (topk_ids_ptr + offs_m[:, None] * stride_topk_ids_m +
                     offs_topk[None, :] * stride_topk_ids_n)

    topk_weights_ptrs = (topk_weights_ptr +
...
def fused_routing(x, w, topk, fused_shared_experts=False):
    ...
    topk_weights = torch.empty((M, _topk),
                               device=x.device,
                               dtype=torch.float32)
